...
public class JedisUtils {
	...
	static {
		...
		try {
			...
			JedisPool pool = new JedisPool(config, DEFAULT_HOST, DEFAULT_PORT, timeout, password);
			pools.add(pool);
		}
	...
	private static JedisPoolConfig initConfig() throws IOException {
		...
		JedisPoolConfig config = new JedisPoolConfig();
		config.setMaxActive(Integer.parseInt(prop.getProperty("maxActive", "1024")));
		config.setMaxIdle(Integer.parseInt(prop.getProperty("maxIdle", "100")));
		poolCount = Integer.parseInt(prop.getProperty("poolCount", "1"));
		host = prop.getProperty("host", prop.getProperty("host", DEFAULT_HOST));
		port = Integer.parseInt(prop.getProperty("port", DEFAULT_PORT + ""));
		password = prop.getProperty("password", DEFAULT_PASSWORD);
		timeout = Integer.parseInt(prop.getProperty("timeout", 2000 + ""));
		return config;
	}
